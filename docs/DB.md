...
| role | USER_ROLE |  | NULL | Роль в системе (админ/создатель форм) |
...
- USER_ROLE (`user_role`):
  - `admin` — роль администратора; доступ к функциям администрирования системы и управлению ролями пользователей.
  - `form_creator` — роль создателя форм; пользователь может создавать формы.
  - `NULL` — роль не назначена; пользователь не получает права администратора и не может создавать формы.

**Правила целостности**
- `user_id` уникален и обязателен.
- `email` уникален и обязателен.
- `etu_id` уникален, если задан.
- Наличие ETU ID само по себе не даёт права создавать формы.
- Создавать формы могут только пользователи с `role = form_creator` или `role = admin`.
