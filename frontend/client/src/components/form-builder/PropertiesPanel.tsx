...
export function PropertiesPanel({ selectedField, selectedIds, updateField, deleteField, deleteSelected, fields }: PropertiesPanelProps) {
  ...
  const textMaxLimit = isText
    ? (selectedField.multiline ? TEXT_MULTILINE_MAX_CHARS : TEXT_SINGLELINE_MAX_CHARS)
    : TEXT_SINGLELINE_MAX_CHARS;
  const textMaxChars = isText
    ? clampTextMaxChars(
        typeof selectedField.maxChars === "number" ? selectedField.maxChars : textMaxLimit,
        textMaxLimit
      )
    : TEXT_SINGLELINE_MAX_CHARS;
          ...
          <>
            <div className="flex items-center justify-between rounded-lg border p-3 shadow-sm mt-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Tooltip delayDuration={0}>
                    <TooltipTrigger asChild>
                      <button
                        type="button"
                        aria-label="Подсказка по многострочному тексту"
                        className="h-5 w-5 rounded-full border border-muted-foreground/40 text-muted-foreground text-[11px] leading-none flex items-center justify-center hover:bg-muted"
                      >
                        ?
                      </button>
              ...
              <Switch
                checked={!!selectedField.multiline}
                onCheckedChange={(checked) => {
                  const prevLimit = selectedField.multiline
                    ? TEXT_MULTILINE_MAX_CHARS
                    : TEXT_SINGLELINE_MAX_CHARS;
                  const nextLimit = checked ? TEXT_MULTILINE_MAX_CHARS : TEXT_SINGLELINE_MAX_CHARS;
                  let nextMaxChars: number;

                  if (typeof selectedField.maxChars === "number") {
                    nextMaxChars =
                      selectedField.maxChars === prevLimit
                        ? nextLimit
                        : clampTextMaxChars(selectedField.maxChars, nextLimit);
                  } else {
                    nextMaxChars = nextLimit;
                  }

                  updateField(selectedField.id, { multiline: checked, maxChars: nextMaxChars });
                }}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  updateField(selectedField.id, { maxChars: clampTextMaxChars(nextValue, textMaxLimit) });
                }}
              />
              <p className="text-xs text-muted-foreground">
